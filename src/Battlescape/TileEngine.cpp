...
namespace
{
// Inverse-square clamped falloff curve sharpness. Single tunable global,
// applied to every light source. Smaller values flatter (more linear);
// larger values concentrate brightness at the centre. The on-screen falloff
// is computeFalloff() below.
	...
	iterateTiles(
		...
		[&](Tile* tile)
		{
			...
			auto tryMapData = [&](TilePart part)
			{
				...
				if (md && md->getEffectiveLightRadius() > currLight && md->isLitInstance(tile->getPosition(), part))
				{
...
void TileEngine::calculateTerrainItems(MapSubset gs)
{
	...
	iterateTiles(
		...
		[&](Tile* tile)
		{
			...
			{
				if (bi->getGlow() && bi->getRules()->isLitInstance(bi->getId()))
				{
...
void TileEngine::calculateUnitLighting(MapSubset gs)
{
	for (BattleUnit *unit : *_save->getUnits())
	{
		...
		// Realistic-lighting tuning: cap unit-emitted light power to prevent over-lighting
		// under LOS-traced propagation. Vanilla soldier flashlights and unit fire reach 15
		// tiles because they wrap around walls; with LOS active, the same power floods rooms
		// with over-saturated colour. The cap applies to whichever source won the per-unit
		// competition (personal light, hand-weapon glow, or unit on fire).
		if (Options::oxceBattleRealisticLighting && currLight > realisticUnitLightPower)
		{
			currLight = realisticUnitLightPower;
		}
